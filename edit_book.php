...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($title) || empty($author) || $quantity <= 0) {
        ...
        $stmt = $pdo->prepare("UPDATE books SET 
                              ...
                              description = ?, category = ?, location = ?, status = ?
                              WHERE id = ?");
        $stmt->execute([
            $title, $author, $isbn, $publisher, $publication_year, $quantity,
            $new_available_quantity, $description, $category, $location, $status,
            $book_id
        ]);

        log_activity($librarian_id, 'edit_book', "Edited book: $title (ID: $book_id)");
        ...
        <main class="content-wrapper">
            <section class="section">
                ...
                <div class="info-box">
                    ...
                    Current available copies: <?php echo htmlspecialchars($book['available_quantity']); ?> of <?php echo htmlspecialchars($book['quantity']); ?>
                </div>
                
                <form method="POST" action="edit_book.php?id=<?php echo $book_id; ?>" class="book-form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="category-select-container">
                            <select id="category" name="category" class="category-select">
                                ...
                                <?php foreach ($categories as $cat): ?>
                                    <option value="<?php echo htmlspecialchars($cat['name']); ?>" 
                                        <?php if ($book['category'] == $cat['name']) echo 'selected'; ?>>
                            ...
                            <input type="text" id="new_category" name="new_category" class="category-input" 
                                   placeholder="Add new category" value="<?php echo (!in_array($book['category'], array_column($categories, 'name')) && !empty($book['category'])) ? htmlspecialchars($book['category']) : ''; ?>">
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label for="location">Location</label>
                        <input type="text" id="location" name="location" value="<?php echo htmlspecialchars($book['location']); ?>">
                    </div>
                    ...
                    <?php if (!empty($book['cover_image'])): ?>
                        <div class="form-group current-image">
                            ...
                            <img src="assets/images/book_covers/<?php echo htmlspecialchars($book['cover_image']); ?>" alt="Current Book Cover">
                        </div>
                    <?php endif; ?>
