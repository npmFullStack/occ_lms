...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'approve') {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO borrow_records 
                                  ...
                                  VALUES (?, ?, NOW(), ?, ?)");
            $stmt->execute([
                $request['book_id'], $request['user_id'], $due_date, $librarian_id
            ]);
            ...
            log_activity($librarian_id, 'approve_request', "Approved request #$request_id for book: {$request['book_title']}");
    ...
    // Process rejection
        ...
        log_activity($librarian_id, 'reject_request', "Rejected request #$request_id for book: {$request['book_title']}");
...
$processed_requests = $pdo->query("SELECT br.*, b.title as book_title, 
                            ...
                            <div class="request-card">
                                ...
                                <div class="request-meta">
                                    ...
                                    <span>
                                        ...
                                        Available: <?php echo htmlspecialchars($request['available_quantity']); ?>
                                    </span>
                                </div>
                        ...
                        <tbody>
                            <?php foreach ($processed_requests as $request): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($request['processed_by_name'] ?? 'System'); ?></td>
                                    <td><?php echo date('M j, Y g:i A', strtotime($request['processed_date'])); ?></td>
