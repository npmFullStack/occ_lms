...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  if (isset($_POST["login"])) {
    ...
    $stmt = $conn->prepare(
      "SELECT id FROM users WHERE username = :username OR id_number = :id_number"
    );
    $stmt->bindParam(":username", $username);
    $stmt->bindParam(":id_number", $id_number);
    $stmt->execute();
    if ($stmt->rowCount() > 0) {
      $errors[] = "Username or ID number already exists";
    }
    ...
    if (empty($errors)) {
      ...
      $stmt = $conn->prepare(
        "INSERT INTO users (username, id_number, password, role, first_name, last_name) VALUES (:username, :id_number, :password, :role, :first_name, :last_name)"
      );
      $stmt->bindParam(":username", $username);
      $stmt->bindParam(":id_number", $id_number);
